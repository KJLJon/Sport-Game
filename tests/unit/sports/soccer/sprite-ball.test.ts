 ...
 * no ball atlas has been built. Mirrors `basketball/sprite-ball.test.ts`.
 */
import { describe, expect, it } from 'vitest';
import { World, type EntityId } from '@/engine/world.ts';
import { buildSoccerAtlases, spriteRenderer } from '@/sports/soccer/sprite-art.ts';
...
function arena(): World {
  return new World({ width: PITCH.length, height: PITCH.width, cellSize: 5, capacity: 8 });
}

function spawnBall(world: World, x: number, y: number, z = 0, radius = 0.11): EntityId {
  return world.spawn({ x, y, z, radius, kind: 1, team: -1 });
}

function renderer() {
  const { factory } = recordingOffscreen();
  return spriteRenderer(buildSoccerAtlases(factory));
}

/** A renderer built from an atlas with no `ball` field, the T-13.4 fallback path. */
function rendererWithoutBall() {
  const { factory } = recordingOffscreen();
  const { teams, keeper } = buildSoccerAtlases(factory);
  return spriteRenderer({ teams, keeper });
}

describe('the soccer sprite ball', () => {
  it('costs exactly one drawImage for the ball, plus its shadow', () => {
    const world = arena();
    const ball = spawnBall(world, 20, 15, 1);
