...
- **Task:** T-3.16 — Roster and full-backup export/import (T-3.14 and T-3.15 delegated, in flight)
- **Status:** in_progress
- **Started:** 2026-07-28
- **Branch commit:** (see `git log`)
- **Done so far:** T-3.1 through T-3.13 are `done`. T-3.14 (starter roster, `haiku`) and T-3.15
  (roster import, `sonnet`) are running as agents; neither may touch `src/app/routes.ts`.
- **Next step:** T-3.16 (backup export/import with change preview), then **T-3.17** — replace
  `rollRatings()` and the local `AthleteRatings` type in `sports/basketball/index.ts` with real
  athletes, and run `pnpm balance` afterwards. That is the only thing that will say whether the
  sport survived being handed real rosters.
...
| 3 | Athletes, cross-sport ratings, roster | 17 | 13 | `in_progress` | v0.2 |
