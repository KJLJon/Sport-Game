...
export function buildBasketballAtlases(
  ...
  return {
    ...
    ball: buildBallAtlas(
      BASKETBALL_BALL,
      basketballBallPalette({ ball: ballColors.ball, seam: ballColors.ballSeam }),
      createOffscreen,
    ),
  };
}
...
// Same rim-height reference `art.ts`'s disc ball shadow uses, so the two renderers agree on how
// high "high" looks (the rim sits at 3.05 m).
const BALL_RISE_FOR_MIN_SHADOW = 3.5;
const BALL_MIN_SHADOW_SCALE = 0.3;
/** `13` §3.2: the sprite scales up to 1.4× at height — half of the altitude pair, with the shadow. */
const BALL_MAX_SPRITE_SCALE = 1.4;
/** World units the sprite climbs the screen per world unit of height, matching soccer's own lift. */
